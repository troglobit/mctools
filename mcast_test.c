...
int main(int argc, char* argv[])
{
   ...
   struct sockaddr_ll addr;     /* Added by Jocke */
   int iface = 1;               /* Use eth1 */
   ...
   mcast_group.sin_port = htons((unsigned short int)strtol(argv[2], 
                                                           NULL, 0));
   ...
   if (setsockopt(recv_s, SOL_SOCKET, SO_REUSEADDR, &yes, 
                  sizeof(yes)) < 0) {
      perror("reuseaddr setsockopt");
      exit(1);
   }
 
   if (bind(recv_s, (struct sockaddr*)&mcast_group, 
            sizeof(mcast_group)) < 0) {
      perror ("bind");
      exit(1);
   }
#else   /* Jocke alternative to bind to an interface instead. */
   recv_s = socket (PF_PACKET, SOCK_RAW, htons (ETH_P_ALL));
   if (-1 == recv_s)
   {
      perror("Could not create socket(PF_PACKET, SOCK_RAW)");
      exit (1);
   }
   ...
   {
      perror("Failed binding to interface");
      exit (1);
   }
#endif
